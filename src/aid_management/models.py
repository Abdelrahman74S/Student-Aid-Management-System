...
class SupportCycle(models.Model):
    ...
    cycle_id = models.UUIDField(
        ...
        unique=True, verbose_name=_("معرّف الدورة")
    )
    serial_number = models.CharField(
        max_length=20, unique=True, blank=True,
        default='', verbose_name=_("رقم الدورة")
    )
    name = models.CharField(
        max_length=255, verbose_name=_("اسم الدورة")
    )
    academic_year = models.CharField(
        max_length=20, verbose_name=_("العام الدراسي")
    )
    ...
    reserved_budget = models.DecimalField(
        max_digits=12, decimal_places=2,
        default=0, verbose_name=_("الميزانية المحجوزة")
    )
    disbursed_budget = models.DecimalField(
        max_digits=12, decimal_places=2,
        default=0, verbose_name=_("الميزانية المصروفة")
    )
    application_start_date = models.DateTimeField(
        verbose_name=_("تاريخ بداية التقديم")
    )
    application_end_date = models.DateTimeField(
        verbose_name=_("تاريخ نهاية التقديم")
    )
    ...
    scoring_rules_snapshot = models.JSONField(
        ...
        verbose_name=_("نسخة قواعد التقييم")
    )
    is_locked = models.BooleanField(
        default=False, verbose_name=_("مقفلة")
    )
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        null=True, related_name='created_cycles',
    ...
    def is_open_for_application(self):
        ...
        return (
            ...
            self.application_start_date <= now <= self.application_end_date
        )
    ...
    def is_open_for_review(self):
        ...
        now = timezone.now()
        return (
            self.status == 'UNDER_REVIEW' and
            not self.is_locked and
            self.review_start_date <= now <= self.review_end_date
        )
    ...
    def clean(self):
        ...
        if self.review_start_date and self.review_end_date:
            ...
            if self.application_end_date and self.review_start_date <= self.application_end_date:
    ...
    def generate_serial(self):
        with transaction.atomic():
            ...
            count = SupportCycle.objects.select_for_update().filter(
...
class AidApplication(models.Model):
    ...
    id = models.UUIDField(
        ...
        editable=False, verbose_name=_("المعرف")
    )
    serial_number = models.CharField(
        max_length=50, unique=True, blank=True,
        default='', verbose_name=_("رقم الطلب")
    )
    student = models.ForeignKey(
        StudentProfile, on_delete=models.CASCADE,
        related_name='applications', verbose_name=_("الطالب")
    )
    cycle = models.ForeignKey(
        'SupportCycle', on_delete=models.CASCADE,
        related_name='applications', verbose_name=_("دورة الدعم")
    )
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES,
        default='DRAFT', verbose_name=_("الحالة")
    ...
    user_agent = models.TextField(
        blank=True, verbose_name=_("معلومات المتصفح")
    )
    ...
    is_locked = models.BooleanField(
        default=False, verbose_name=_("مقفل")
    )
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name=_("تاريخ الإنشاء")
    )
...
class ScoringRule(models.Model):
    ...
    rule_id = models.UUIDField(
        default=uuid.uuid4, editable=False,
        unique=True, verbose_name=_("معرّف القاعدة")
    )
    cycle = models.ForeignKey(
        SupportCycle, on_delete=models.CASCADE,
        related_name='rules', verbose_name=_("دورة الدعم")
    )
    criteria_type = models.CharField(
        max_length=30, choices=CRITERIA_CHOICES,
        verbose_name=_("نوع المعيار")
    )
    condition = models.JSONField(
        verbose_name=_("شرط التطبيق"),
        help_text=_("مثال: {'min': 0, 'max': 3000}")
    )
    description = models.TextField(
        blank=True, verbose_name=_("وصف القاعدة")
    )
    points = models.IntegerField(verbose_name=_("عدد النقاط"))
    weight = models.DecimalField(
        max_digits=4, decimal_places=2,
        default=1.0, verbose_name=_("الوزن")
    )
    priority = models.IntegerField(
        default=0, verbose_name=_("الأولوية")
    )
    is_active = models.BooleanField(
        default=True, verbose_name=_("مفعل")
    )
    ...
    def __str__(self):
        return _(
            "%(criteria)s — %(points)s نقطة (وزن: %(weight)s)"
        ) % {
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['cycle', 'criteria_type', 'is_active'])
        ]
...
class CommitteeReview(models.Model):
    ...
    review_id = models.UUIDField(
        default=uuid.uuid4, editable=False,
        unique=True, verbose_name=_("معرّف المراجعة")
    )
    application = models.ForeignKey(
        AidApplication, on_delete=models.CASCADE,
        related_name='reviews', verbose_name=_("الطلب")
    )
    reviewer = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        null=True, related_name='committee_reviews',
        verbose_name=_("المراجع")
    )
    status = models.CharField(
        max_length=20, choices=REVIEW_STATUS_CHOICES,
        default='DRAFT', verbose_name=_("حالة المراجعة")
    )
    conflict_of_interest = models.BooleanField(
        default=False, verbose_name=_("تعارض مصالح")
    )
    dimension_scores = models.JSONField(
        default=dict, verbose_name=_("درجات التقييم"),
        help_text=_("مثال: {'income_tier': 10, 'gpa': 5}")
    )
    total_score = models.DecimalField(
        max_digits=8, decimal_places=2,
        default=0, verbose_name=_("المجموع الكلي")
    )
    qualitative_notes = models.TextField(
        blank=True, verbose_name=_("ملاحظات نوعية")
    )
    review_timestamp = models.DateTimeField(
        auto_now_add=True, verbose_name=_("وقت المراجعة")
    )
    is_submitted = models.BooleanField(
        default=False, verbose_name=_("تم التقديم")
    )
    ...
    def clean(self):
        if self.conflict_of_interest and self.is_submitted:
            raise ValidationError(
                _("لا يمكن تقديم المراجعة في حالة وجود تعارض في المصالح.")
            )
        if self.is_submitted and not self.dimension_scores:
            raise ValidationError(
                _("لا يمكن تقديم مراجعة بدون درجات تقييم.")
            )
    ...
    def calculate_total(self):
        ...
        weight_map = {
            rule.criteria_type.lower(): float(rule.weight)
            for rule in rules
        }
        self.total_score = sum(
            score * weight_map.get(dimension.lower(), 1)
    ...
    def submit(self):
        ...
        self.save(update_fields=[
            'total_score', 'is_submitted', 'status', 'updated_at'
        ])
...
class BudgetAllocation(models.Model):
    ...
    allocation_id = models.UUIDField(
        default=uuid.uuid4, editable=False,
        unique=True, verbose_name=_("معرّف التخصيص")
    )
    cycle = models.ForeignKey(
        SupportCycle, on_delete=models.CASCADE,
        related_name='allocations', verbose_name=_("دورة الدعم")
    )
    application = models.ForeignKey(
        AidApplication, on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name='allocations', verbose_name=_("الطلب")
    )
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES,
    ...
    amount_disbursed = models.DecimalField(
        max_digits=10, decimal_places=2,
        default=0, verbose_name=_("المبلغ المصروف")
    )
    ...
    def disburse(self, amount, disbursement_date=None, disbursed_by=None):
        ...
        with transaction.atomic():
            ...
            self.cycle.disbursed_budget += amount
            self.cycle.save(update_fields=['disbursed_budget', 'updated_at'])

            self.full_clean()
            self.save(update_fields=[
                'amount_disbursed', 'disbursement_date',
                'disbursed_by', 'status', 'updated_at'
            ])
    ...
    def cancel(self):
        ...
        with transaction.atomic():
            if self.status == 'PENDING':
                self.cycle.reserved_budget -= self.amount_allocated
                self.cycle.save(update_fields=['reserved_budget', 'updated_at'])
