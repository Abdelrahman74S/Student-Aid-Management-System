...
from accounts.mixins import AuditorRequiredMixin
...
# ==========================================
class AuditStaffRequiredMixin(AuditorRequiredMixin):
    """يسمح للمراقبين، طاقم العمل، والمديرين بالوصول"""
    def test_func(self):
        user = self.request.user
        return user.is_authenticated and (
            user.role == UserRoles.AUDITOR or 
            user.is_staff or 
            user.is_superuser
        )
...
class AuditDashboardView(AuditStaffRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['stats'] = {
            ...
            'unbalanced_cycles': FinancialIntegrityLog.objects.filter(is_balanced=False).count(),
        }
        # حالة التوازن المالي لآخر دورة
        context['integrity_check'] = FinancialIntegrityLog.objects.select_related('cycle').order_by('-timestamp').first()
        
        # توزيع العمليات للرسم البياني
        context['actions_summary'] = DataAuditLog.objects.values('action').annotate(total=Count('action'))
...
class DataAuditListView(AuditStaffRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = DataAuditLog.objects.select_related('user').all().order_by('-timestamp')
...
class OverrideLogListView(AuditStaffRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return SystemOverrideLog.objects.select_related('admin_user', 'application').all().order_by('-timestamp')
...
class BudgetAuditListView(AuditStaffRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = BudgetAuditLog.objects.select_related('cycle', 'user').all().order_by('-timestamp')
...
class ProcessActionListView(AuditStaffRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return ProcessActionLog.objects.select_related('user').all().order_by('-timestamp')